...
function bougeRectangle() {
  ...
  rectangle.style.height = parseInt(rectangle.style.height) + 10 + "px";
    ...
    //rectangle.style.bottom = timePassed / +3 + "px";

    rectangle.x = parseInt(rectangle.style.left);
    rectangle.y = parseInt(rectangle.style.bottom);
    rectangle.width = 10;
    rectangle.height = parseInt(rectangle.style.height);

    block.x = parseInt(block.style.left);
    block.y = parseInt(block.style.bottom);
    block.width = 900;
    block.height = 100;

    console.log(rectangle.x,rectangle.y,"rectangle");
    console.log(block.x,block.y,"block");
    if(isCollide(rectangle,block)) {
...
function toucheEspace(event){
  if(event.keyCode =="32") {
    ...
    //let start = Date.now();
     let timer = setInterval(bougeRectangle, 30);
...
if(parseInt(block.style.left) >= 1000) {
  clearInterval(transInterval);
     block.style.bottom = parseInt(block.style.bottom) - 100 + "px";
  setTimeout(function(){
